...
```bash
python -m actors.worker --trainer-url http://localhost:5001
```
...
python -m actors.worker --trainer-url http://localhost:5001 --mode llm --adapter-name npc_core_pythia_410m_v1
```
...
curl -X POST http://localhost:5001/enqueue_blocks \
  ...
  -d '{"dataset_label":"local","blocks_path":"data/processed/experience_blocks.jsonl","target_adapter":"npc_core_pythia_410m_v1"}'
```
